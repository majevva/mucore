...
if(isset($_GET['aG22'])){
	...
	$acc_Gs = base64_decode($_GET['aG22']);
	$c_s = $core_db2->Execute("Select connectstat from MEMB_STAT where memb___id=?",array($acc_Gs));
	if($c_s->EOF){
		echo 'N/A';
	}else{
		if($c_s->fields[0] == '0'){
	...
	$c_aa = $core_db2->Execute("Select memb___id from memb_info where memb___id=?",array($acc_aA));
	...
	if(!empty($_GET['aMl'])){
		...
		$c_am = $core_db2->Execute("Select mail_addr from memb_info where mail_addr=?",array($acc_aM));
	...
	$c_m = $core_db->Execute("Select mapnumber,mapposx,mapposy from character where mu_id=?",array($acc_Ms));
	if($c_m->EOF){
		echo 'N/A';
	}else{
	...
	function guarico_hex2bin ($RawInput){
		$BinStr = '';
	...
	imagecolortransparent($img, '00ffff');
	ImageGif($img);
