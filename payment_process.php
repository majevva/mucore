 				...
 				while (!feof($fp)) {
 					...
 					if (strcmp ($res, "VERIFIED") == 0) {
 						...
 						if(strtolower($receiver_email) == strtolower($paypal_email)){
 							$check_for_temp = $core_db->Execute("Select amount,currency,memb___id,credits from MUCore_PayPal_Donate_Orders where hash=?",array($item_number));
 								...
 								if (($txn_type == 'web_accept' OR $txn_type == 'subscr_payment') AND $payment_status == 'Completed'){
 									...
 									if($mc_gross == $order_cost){
 										if($currency == $check_for_temp->fields[1]){
 											$check_if_transaction_exist = $core_db->Execute("Select id from MUCore_PayPal_Donate_Transactions where transaction_id=?",array($transaction_id));
 											if($check_if_transaction_exist->RecordCount() <= 0){
 												$order_date_time = time();
 												$insert_txn_id = $core_db->Execute("INSERT INTO MUCore_PayPal_Donate_Transactions (transaction_id,amount,currency,memb___id,credits,order_date,status,payer_email)VALUES(?,?,?,?,?,?,?,?)",array($transaction_id,$mc_gross,$currency,$check_for_temp->fields[2],$check_for_temp->fields[3],$order_date_time,$payment_status,$payer_email));
 												if($insert_txn_id){
 													...
 													$check_for_memb_id = $core_db2->Execute("Select ".MU_COINS_USERID_COLUMN." from ".MU_COINS_TABLE." where ".MU_COINS_USERID_COLUMN."=?",array($check_for_temp->fields[2]));
 													if($check_for_memb_id->EOF){
 														$set_credits = $core_db2->Execute("insert into ".MU_COINS_TABLE." (".MU_COINS_USERID_COLUMN.",".MU_COINS_COLUMN.")VALUES(?,?)",array($check_for_temp->fields[2],$check_for_temp->fields[3]));
 													}else{
 														$set_credits = $core_db2->Execute("Update ".MU_COINS_TABLE." set ".MU_COINS_COLUMN."=".MU_COINS_COLUMN."+?  where ".MU_COINS_USERID_COLUMN."=?",array($check_for_temp->fields[3],$check_for_temp->fields[2]));
 													}
 													if($set_credits){
 									...
 									if($punish == '1'){
 										$update_transaction_info = $core_db->Execute("Update MUCore_PayPal_Donate_Transactions set status=? where transaction_id=?",array($payment_status,$transaction_id));
 										if($update_transaction_info){
 											$bloc_userid = $core_db2->Execute("Update memb_info set bloc_code='1' where memb___id=?",array($check_for_temp->fields[2]));
 											if($bloc_userid){
