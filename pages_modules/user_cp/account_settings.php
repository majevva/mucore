...
if($active == '0'){
	...
	if($verification_config->human_verification_type == 'reCAPTCHA'){
		...
		$jq_cron = $core_db->Execute("Select next_cron from MUCore_Cron_Jobs where cron_id=?",array(trim($settings->cron_job)));
			if(cron_check($jq_cron->fields[0]) == false){
				$jq_cron_up = $core_db->Execute("Update MUCore_Cron_Jobs set next_cron=(".time()."+cron_time_set) where cron_id=?",array(trim($settings->cron_job)));
				...
				while (!$_get_temp_passwords->EOF){
					...
					if($temp_expire_pass_time <= 0){
						$delete_temp_pass = $core_db->Execute("Delete from MUCore_Change_Passwords where id=?",array($_get_temp_passwords->fields[0]));
					}	
		...
		$elems[] = array('name'=>'current_password', 'label'=>''.text_accountsettings_t1.'', 'type'=>'text', 'required'=>true, 'len_min'=>6,'len_max'=>12, 'cont' =>'alpha');
		$elems[] = array('name'=>'new_password', 'label'=>''.text_accountsettings_t2.'', 'type'=>'text', 'required'=>true, 'len_min'=>6,'len_max'=>12, 'cont' =>'alpha');
		$elems[] = array('name'=>'confirm_new_password', 'label'=>''.text_accountsettings_t3.'','type'=>'text', 'required'=>true, 'len_min'=>6,'len_max'=>12, 'cont' =>'alpha','equal'=> array('new_password'));
		...
		if($bot_check == '1'){
			...
			if($core['config']['md5'] == '1'){
				$pass_ver = $core_db2->Execute("Select memb__pwd,mail_addr from MEMB_INFO where memb__pwd = [dbo].[fn_md5](?,?) and memb___id = ?",array($current_password,$user_auth_id,$user_auth_id));
			}elseif ($core['config']['md5'] == '0'){
				$pass_ver = $core_db2->Execute("Select memb__pwd,mail_addr from MEMB_INFO where memb__pwd = ? and memb___id = ?",array($current_password,$user_auth_id));
			}
			if($pass_ver){
				if($pass_ver->EOF){
					...
					$insert_change_password = $core_db->Execute("Insert into MUCore_Change_Passwords(password,email,expire,memb___id,hash) VALUES (?,?,?,?,?)",array($new_password,$pass_ver->fields[1],time()+86400,$user_auth_id,$hash));
...
;
			...
			<tr>
			<td align="right"><input type="submit"  class="btn btn-primary btn-sm" value="Aceptar" onclick="return prse_inputs()"></td>
			<td align="left"><input type="button"  class="btn btn-danger btn-sm" value="Aceptar" onclick="location.href=\''.$core['config']['website_url'].'\'"></td>
			</table>
			</form>';
			...
			*/
			...
			if(empty($_GET['change_password_md5'])){
				...
				$take_pass = $core_db->Execute("Select memb___id,password from MUCore_Change_Passwords where hash=?",array($md5_link));
				if($take_pass->EOF){
					echo msg('0',text_accountsettings_t22);
				}else{
					if($core['config']['md5'] == '1'){
						$pass_up = $core_db2->Execute("Update MEMB_INFO set memb__pwd = [dbo].[fn_md5](?,?) where memb___id=?",array($take_pass->fields[1],$take_pass->fields[0],$take_pass->fields[0]));
					}elseif ($core['config']['md5'] == '0'){
						$pass_up = $core_db2->Execute("Update MEMB_INFO set memb__pwd = ? where memb___id=?",array($take_pass->fields[1],$take_pass->fields[0]));
					}
					if($pass_up){
	...
	if($verification_config->human_verification_type == 'reCAPTCHA'){
		...
		if($bot_check == '1'){
			...
			if($core['config']['md5'] == '1'){
				$pass_ver = $core_db2->Execute("Select memb__pwd from MEMB_INFO where memb__pwd = [dbo].[fn_md5](?,?) and memb___id = ?",array($current_password,$user_auth_id,$user_auth_id));
			}elseif ($core['config']['md5'] == '0'){
				$pass_ver = $core_db2->Execute("Select memb__pwd from MEMB_INFO where memb__pwd = ? and memb___id = ?",array($current_password,$user_auth_id));
			}
			if($pass_ver){
				if($pass_ver->EOF){
					echo msg('0',text_accountsettings_t5);
	...
	echo '	<form name="form" method="post" action="">
			<table border="0" cellspacing="10" cellpadding="0" width="100%" style="margin-top: 10px; margin-bottom: 10px;" align="center">
			<tr>
			<td colspan="3" align="center" class="iRg_line">Cambiar contrase&ntilde;a</td>
			</tr>
			<tr>
			<td align="left" class="iRg_text" style="padding-left: 24px;" width="160">Actual contrase&ntilde;a</td>
			<td align="left" class="iRg_inf"><input class="iRg_input" type="password" name="current_password" maxlength="12"></td>
...
<tr>
<td align="center" colspan="2"><img style="vertical-align:center;" src="get.php?aI" border="0"></td>
</tr>
<tr>
<td align="center" class="iRg_inf">Ingrese el codigo captcha:</td>
...
;
	...
	echo '
			...
			<tr>
			<td align="right"><input type="submit"  class="btn btn-primary btn-sm" value="Aceptar" onclick="return prse_inputs()"></td>
			<td align="left"><input type="button"  class="btn btn-danger btn-sm" value="Cancelar" onclick="location.href=\''.$core['config']['website_url'].'\'"></td>
			</table>
			</form>';
	}
