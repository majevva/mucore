...
if($active == '0'){
	...
	$check_mu_coins = $core_db2->Execute("Select ".MU_COINS_COLUMN." from ".MU_COINS_TABLE." where ".MU_COINS_USERID_COLUMN."=?",array($user_auth_id));
	if($check_mu_coins->EOF){
		$insert_mu_coins_id = $core_db2->Execute("INSERT INTO ".MU_COINS_TABLE." (".MU_COINS_USERID_COLUMN.",".MU_COINS_COLUMN.")VALUES(?,?)",array($user_auth_id,'0'));
...
<td align="right" colspan="2"><input type="button" value="'.button_purchase_credits.'" onclick="location.href=\''.$config->buy_url.'\'">
  ...
  ';
	$select_transactions = $core_db->Execute("Select transaction_id,amount,currency,credits,order_date,status from MUCore_PayPal_Donate_Transactions where memb___id=? order by order_date desc",array($user_auth_id));
	$count=0;
