...
if(isset($_GET['m'])){
	if($_GET['m'] == '1'){
		if(isset($_GET['unban'])){
			...
			$check_ban = $core_db->Execute("Select type,ban_id from MUCore_ban where type='1' and id=?",array($banid));
			if($check_ban->EOF){
				echo notice_message_admin('No character found in ban list.','0','1','0');
			}else{
						$set_unban = $core_db->Execute("Update character set ctlcode='0' where mu_id=?",array($check_ban->fields[1]));
						if($set_unban){
							$delete_from_list = $core_db->Execute("Delete from MUCore_ban where id=? and type='1'",array($banid));
							if($delete_from_list){
		  ...
		  if(isset($_POST['add'])){
			if(empty($_POST['ban_id']) || $_POST['ban_period'] == 'x'){
				...
				$check_ban_id = $core_db->Execute("Select mu_id,name,AccountID from character where name=?",array($banid));
				if($check_ban_id->EOF){
					echo notice_message_admin('No character found.','0','1','0');
				}else{
					$check_if_ban_exist = $core_db->Execute("Select ban_id from MUCore_ban where ban_id=? and type='1'",array($check_ban_id->fields[0]));
					if(!$check_if_ban_exist->EOF){
						...
						$set_ban = $core_db->Execute("Update character set ctlcode='1' where mu_id=?",array($check_ban_id->fields[0]));
						if($set_ban){
							$insert_ban = $core_db->Execute("INSERT INTO MUCore_Ban (ban_id,type,ban_date,ban_expire,reason,ban_name,ban_permanent,author) VALUES (?,?,?,?,?,?,?,?)",array($check_ban_id->fields[0],'1',time(),$ban_expire,$reason,$check_ban_id->fields[1],$ban_permanent,$nickname));
							if($insert_ban){
	...
	<td align="left" class="panel_text_alt2" width="50%"><input type="text" class="form-control" name="ban_id" ></td>
	</tr>
	<tr>
	...
	<td align="left" class="panel_text_alt2" width="50%"><input type="text" class="form-control" name="ban_reason"></td>
	</tr>
	
		<tr>
	<td align="center" class="panel_buttons" colspan="2">
	<input type="hidden" name="add">
	<input type="submit" class="btn btn-block btn-warning btn-200" value="Ban Character" onclick="return ask_url(\'Are you sure?\')"></td>
	</tr>
	</table>
	</form>';
...
if(isset($_GET['permanent'])){
	...
	$user = $core_db->Execute("Select id,ban_name,reason,ban_date,ban_expire,ban_permanent,ban_id,type,author from MUCore_Ban where ban_permanent='0' and type='1' order by ban_expire asc");
				...
				while (!$user->EOF){
						...
						if($time_left <= 0){
							$set_unban = $core_db->Execute("Update character set ctlcode='0' where mu_id=?",array($user->fields[6]));
							if($set_unban){
								$delete_from_list = $core_db->Execute("Delete from MUCore_ban where id=? and type=?",array($user->fields[0],$user->fields[7]));
							}
						}
					}
		...
		if(isset($_GET['unban'])){
			$banid = safe_input($_GET['unban'],'');
			$check_ban = $core_db->Execute("Select type,ban_id from MUCore_ban where type='0' and id=?",array($banid));
			if($check_ban->EOF){
				echo notice_message_admin('No account found in ban list.','0','1','0');
			}else{
						$set_unban = $core_db2->Execute("Update memb_info set bloc_code='0' where memb_guid=?",array($check_ban->fields[1]));
						if($set_unban){
							$delete_from_list = $core_db->Execute("Delete from MUCore_ban where id=? and type='0'",array($banid));
							if($delete_from_list){
			...
			if(empty($_POST['ban_id']) || $_POST['ban_period'] == 'x'){
				...
				$reason = addslashes($_POST['ban_reason']);
				$period = addslashes($_POST['ban_period']);
				$check_ban_id = $core_db2->Execute("Select memb_guid,memb___id from memb_info where memb___id=?",array($banid));
				if($check_ban_id->EOF){
					echo notice_message_admin('No account found.','0','1','0');
				}else{
					$check_if_ban_exist = $core_db->Execute("Select ban_id from MUCore_ban where ban_id=? and type='0'",array($check_ban_id->fields[0]));
					if(!$check_if_ban_exist->EOF){
						...
						$set_ban = $core_db2->Execute("Update memb_info set bloc_code='1' where memb_guid=?",array($check_ban_id->fields[0]));
						if($set_ban){
							$insert_ban = $core_db->Execute("INSERT INTO MUCore_Ban (ban_id,type,ban_date,ban_expire,reason,ban_name,ban_permanent,author) VALUES (?,?,?,?,?,?,?,?)",array($check_ban_id->fields[0],'0',time(),$ban_expire,$reason,$check_ban_id->fields[1],$ban_permanent,$nickname));
							if($insert_ban){
	...
	<td align="center" class="panel_buttons" colspan="2">
	<input type="hidden" name="add">
	<input type="submit" class="btn btn-block btn-warning btn-200" value="Ban Account" onclick="return ask_url(\'Are you sure?\')"></td>
	</tr>
	</table>
	</form>';
...
if(isset($_GET['permanent'])){
	...
	$user = $core_db->Execute("Select id,ban_name,reason,ban_date,ban_expire,ban_permanent,ban_id,type,author from MUCore_Ban where ban_permanent='0' and type='0' order by ban_expire asc");
				...
				while (!$user->EOF){
						...
						$time_left = $user->fields[4]-time();
						if($time_left <= 0){
							$set_unban = $core_db2->Execute("Update memb_info set bloc_code='0' where memb_guid=?",array($user->fields[6]));
							if($set_unban){
								$delete_from_list = $core_db->Execute("Delete from MUCore_ban where id=? and type=?",array($user->fields[0],$user->fields[7]));
							}
						}
...
<tr>
<td align="left" class="panel_text_alt1" width="30%" valign="top"><a href="index.php?get=ban_manager&m=1">[Character]</a></td>
<td align="left" class="panel_text_alt1" width="70%" valign="top">Ban Manager for characters.</td>
</tr>
<tr class="even">
<td align="left" class="panel_text_alt1" width="30%" valign="top"><a href="index.php?get=ban_manager&m=0">[Account]</a></td>
