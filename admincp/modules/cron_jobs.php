...
if(isset($_GET['mod'])){
	if($_GET['mod'] == 'new'){
		if(isset($_POST['new'])){
			if($_POST['time'] == 'x' || empty($_POST['title']) || empty($_POST['cron_id'])){
				...
				$check_for_id = $core_db->Execute("Select id from MUCore_Cron_Jobs where cron_id=?",array($cron_id));
				if(!$check_for_id->EOF){
					echo notice_message_admin('There is already ony cron job with this cron job id: <b>'.$cron_id.'</b>.','0','1','0');
				}else{
					$total_time = $time*3600;
					$insert_cron = $core_db->Execute("Insert into MUCore_Cron_Jobs (name,cron_id,next_cron,cron_time_set) VALUES (?,?,?,?)",array($title,$cron_id,time()+$total_time,$total_time));
					if($insert_cron){
	...
	<td align="left" class="panel_text_alt2" width="50%"><input type="text" class="form-control"  name="cron_id" ></td>
	</tr>
	
	<tr>
	<td align="center" class="panel_buttons" colspan="2">
	<input type="hidden" name="new">
	<input type="submit" class="btn btn-block btn-success btn-200" value="Add Cron Job"></td>
	</tr>
		...
		$take_info = $core_db->Execute("Select name,next_cron,cron_id,cron_time_set from MUCore_Cron_Jobs where id=?",array($id));
		if(isset($_POST['edit'])){
			if($_POST['time'] == 'x' || empty($_POST['title']) || empty($_POST['cron_id'])){
				echo notice_message_admin('Error some fields where left blank.','0','1','0');
			}else{
				...
				$time = safe_input($_POST['time'],'');
					...
					$total_time = $time*3600;
					$update_cron = $core_db->Execute("Update MUCore_Cron_Jobs set name=?,cron_time_set=?,cron_id=? where id=?",array($title,$total_time,$cron_id,$id));
					if($update_cron){
	...
	<tr>
	<td align="left" class="panel_text_alt1" width="50%">Enter cron job title.</td>
	<td align="left" class="panel_text_alt2" width="50%"><input type="text" class="form-control"  name="title" value="'.$take_info->fields[0].'"></td>
	</tr>
	...
	<tr>
	<td align="left" class="panel_text_alt1" width="50%">Enter cron job ID. Use only letters and numbers.</td>
	<td align="left" class="panel_text_alt2" width="50%"><input type="text" class="form-control"  name="cron_id" value="'.$take_info->fields[2].'"></td>
	</tr>
	...
	if(isset($_GET['delete'])){
		...
		$delete_cron = $core_db->Execute("Delete from MUCore_Cron_Jobs where id=?",array($id));
		if($delete_cron){
...
<input type="button" class="btn btn-block btn-success btn-200" value="Add New Cron Job" onclick="location.href=\'index.php?get=cron_jobs&mod=new\'"></td>
</tr>
			</table>';
	}
}
