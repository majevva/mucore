...
<b>reCAPTCHA&#8482;</b><br>
A CAPTCHA is a program that protects websites against bots by generating and grading tests that humans can pass but current computer programs cannot. For example, humans can read distorted text, but current computer programs can\'t.</em><br><a href="https://www.google.com/recaptcha/admin" target="blank">Visit reCAPTCHA&trade; site for more infos.</a>
</td>
<td align="left" class="panel_text_alt2" width="45%" valign="top">';
		foreach ($human_verifications as $verification_id => $verification_html){
			if($get_config->human_verification_type == $verification_id){
				echo '<label><input type="radio" class="minimal" value="'.$verification_id.'" name="human_verification_type" checked> '.$verification_html.'</label><br>';
			}else{
				echo '<label><input type="radio" class="minimal" value="'.$verification_id.'" name="human_verification_type"> '.$verification_html.'</label><br>';
			}
...
<tr>
<td align="left" class="panel_text_alt1" width="45%" valign="top">Public key provided to you by <a href="https://www.google.com/recaptcha/admin" target="blank">reCAPTCHA&trade;.</a>
</td>
<td align="left" class="panel_text_alt2" width="45%" valign="top"><input type="text" class="form-control" name="reCAPTCHA_public_key" value="'.$get_config->reCAPTCHA_public_key.'" size="40"></td>
</tr>


<tr>
<td align="left" class="panel_title_sub" colspan="2">reCAPTCHA&#8482; Secret Key</td>
</tr>
<tr>
<td align="left" class="panel_text_alt1" width="45%" valign="top">Private key provided to you by <a href="https://www.google.com/recaptcha/admin" target="blank">reCAPTCHA&trade;.</a>
</td>
<td align="left" class="panel_text_alt2" width="45%" valign="top"><input type="text" class="form-control" name="reCAPTCHA_private_key" value="'.$get_config->reCAPTCHA_private_key.'" size="40"></td>
</tr>
...
<td align="left" class="panel_text_alt2" width="45%" valign="top">';
		foreach ($recaptcha_themes as $theme_id => $theme_html){
			if($get_config->reCAPTCHA_theme == $theme_id){
				echo '<label><input type="radio" class="minimal" value="'.$theme_id.'" name="reCAPTCHA_theme" checked> '.$theme_html.'</label><br>';
			}else{
				echo '<label><input type="radio" class="minimal" value="'.$theme_id.'" name="reCAPTCHA_theme"> '.$theme_html.'</label><br>';
			}
