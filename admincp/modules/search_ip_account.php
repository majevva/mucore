...
<td align="left" class="panel_title_sub2" width="50">Controls</td>
				...
				if($_POST['search_t'] == '1'){
				$user = $core_db->Execute("Select memb___id,ip from memb_stat where memb___id=?",array($userid));
				if($user->EOF){
					$not_found = '1';
				}
				}elseif ($_POST['search_t'] == '0'){
					$user = $core_db->Execute("Select top 100 memb___id,ip from memb_stat where memb___id like ?",array('%'.$userid.'%'));
				}
