<?
$log_dir = '../engine/logs/gmcp/mucoins_manager';
if(isset($_GET['delete_logs_date'])){
	if(empty($_GET['delete_logs_date'])){
		...
		if(unlink($log_dir.'/'.$log_id.'.log')){
			echo notice_message_admin('Logs from  '.str_replace("_"," ",$log_id).' successfully edited',1,0,'index.php?get=logs_gm_credits_manager');
		}
	}
	...
	echo notice_message_admin('<b>'.$count.'</b> logs have been successfully deleted',1,0,'index.php?get=logs_gm_credits_manager');
}else{
...
<input type="submit" class="btn btn-block btn-danger btn-200" value="Delete All Logs" onclick="return ask_form(\'Are you sure you want to delete all logs?\')"></td>
</tr>
...
echo '
<div align="right" style="width: 90%; margin-bottom: 2px;"><a href="index.php?get=logs_gm_credits_manager">[View Today Logs]</a></div>
<div align="left" style="width: 90%; margin-bottom: 2px;"><form name="form1">
  <select name="date" onChange="MM_jumpMenu(\'parent\',this,0)">
  <optgroup label="---------------------------------------">
  <option value="index.php?get=logs_gm_credits_manager" selected="selected">Today ('.date('F j Y').')</option>
		<optgroup label="---------------------------------------">';
    ...
    if ($dh = opendir($log_dir)) {
        while (($file = readdir($dh)) !== false) {
        	if($file != '.' && $file != '..'){
        		...
        		if($date == $file){
        			echo '<option value="index.php?get=logs_gm_credits_manager&date='.$file.'" selected="selected">'.str_replace("_"," ",$file).'</option>';
        		}else{
        			echo '<option value="index.php?get=logs_gm_credits_manager&date='.$file.'">'.str_replace("_"," ",$file).'</option>';
        		}
        	}
...
<tr>
 <td align="center" class="panel_title" colspan="2">Game Master MU Coins Manager Logs</td>
</tr>
<tr>
...
<input type="button" value="Delete '.str_replace("_"," ",$date).' Logs" onclick="ask_url(\'Are you sure?\',\'index.php?get=logs_gm_credits_manager&delete_logs_date='.$date.'\')"></td>
</tr>';
	}
